...
void FetchStage::read() {
    ...
    this->instruction = this->ptrCore->read_PC();
...
void FetchStage::reset() {
    // Reset the output latches
    this->instructionLatch = 0;
}
    ...
    if (this->check_valid()) {
        ...
        switch (masked_instr) {
            ...
            case 0b11011:
                ...
                break;
            default:
                std::cout << "Instruction format invalid! \n";
                exit(0);
        }
...
void DecodeStage::reset() {
    this->rd_validLatch = 0;
    this->instr_typeLatch = 0;
    this->instr_formatLatch = 0;
    this->immidiateLatch = 0;
    this->src1_valueLatch = 0;
    this->src2_valueLatch = 0;
}
...
MemoryAccessStage::MemoryAccessStage(Core *cpu_core) {
...
bool MemoryAccessStage::check_valid() {
    if (this->rd_valid == 0 
        && this->result == 0 
        && this->instr_format == 0 
        && this->src2_value == 0) {
...
bool WritebackStage::check_valid() {
    return this->rd_valid == true;
}
